Mero Sarkar Guide
Mero Sarkar Guide is a digital platform designed to help Nepali citizens understand and complete government office procedures efficiently. The system provides step-by-step guidance, AI-based support, and automated form filling using OCR technology.
Key Features
Government Service Guidance: Clear, structured information about procedures such as voter ID registration, citizenship, and passport application.
AI Chat Support: Interactive chat assistant that answers user queries about requirements and processes.
Smart Form Automation: Upload documents, extract details through OCR, and auto-fill relevant government forms.
User-Friendly Interface: Minimal design supporting both Nepali and English languages.
Tech Stack
Frontend: React, TailwindCSS
Backend: FastAPI, Node.js
Database: MongoDB
OCR & AI: OpenCV, Tesseract.js, Pytesseract
Authentication (Planned): Google / Nagarik App
